...
* [x] **Docker Hub(또는 AWS ECR) 레포지토리 생성** (빌드된 이미지를 저장할 공간)
* [x] **GitHub Repository Secret 설정** (AWS 키, Docker Hub 패스워드 등 민감한 정보 안전하게 저장)
* [x] **CI (지속적 통합) 워크플로우 작성** (`.github/workflows/ci-cd.yml`)
* [x] 코드 푸시 시 자동으로 테스트 및 Docker 이미지 빌드
* [x] 빌드된 이미지를 Docker Hub에 Push


* [x] **CD (지속적 배포) 워크플로우 추가**
* [x] EC2 서버에 SSH로 원격 접속하는 스크립트 작성
* [x] 최신 Docker 이미지 다운로드(Pull) 및 기존 컨테이너 재시작(Restart) 자동화


* [x] **자동 배포 확인** (코드를 수정 후 GitHub에 Push했을 때, 실제 서비스에 자동으로 반영되는지 최종 확인)

#### 🟩 5단계: 부하 테스트(Load Test) 체험 (인프라 성능 검증)

* [x] **부하 테스트 툴 선정 및 설치** (Locust — `requirements-locust.txt`, `locustfile.py`)
* [x] **테스트 시나리오 작성**
* [x] 메인 페이지/메모 목록 조회 (읽기 집중 부하) — `MemoReader`
* [x] 짧은 시간 내에 대量의 메모 작성 요청 (쓰기 집중 부하) — `MemoWriter`


* [x] **점진적 부하 테스트 실행** (가상 유저 수를 10명 -> 50명 -> 100명으로 늘려가며 서버 반응 확인) — `scripts/run-loadtest.sh`
* [x] **인프라 모니터링 및 병목 지점 파악**
* [x] EC2 내부에서 `top` 또는 `htop` 명령어로 CPU/Memory 사용량 관찰
* [x] 부하가 걸렸을 때 Redis 캐싱을 적용한 경우와 적용하지 않은 경우의 응답 속도(Latency) 차이 비교 분석


* [x] **테스트 결과 정리 및 회고** (서버가 버틸릭 있는 최대 RPS(초당 요청 처리) 파악 및 개선점 정리) — `learn/Step5.md` 표 작성
